export const translations = {
  tr: {
    common: {
      save: "Kaydet",
      cancel: "İptal",
      loading: "Yükleniyor...",
      edit: "Düzenle",
      delete: "Sil",
      update: "Güncelle",
      yes: "Evet",
      no: "Hayır",
      general: "Genel",
      system: "Sistem",
      search: "Ara...",
      close: "Kapat",
      other: "Diğer",
      success: "Başarılı",
      serverUnreachable: "Sunucuya şu an ulaşılamıyor, lütfen internet bağlantınızı kontrol edin.",
      deleting: "Siliniyor...",
      yesDelete: "Evet, Sil",
      paying: "Ödeniyor...",
      yesPay: "Evet, Öde",
      unspecified: "Belirtilmemiş"
    },
    sidebar: {
      dashboard: "Anasayfa",
      invoices: "Faturalar",
      staff: "Personel Yönetimi",
      reports: "Raporlar ve Analiz",
      settings: "Ayarlar",
      logout: "Çıkış Yap"
    },
    topbar: {
      searchPlaceholder: "Fatura, satıcı veya açıklama ara...",
      notifications: "Bildirimler",
      markAllRead: "Tümünü Okundu Yap",
      clearAll: "Tümünü Temizle",
      mute: "Sesi Kapat",
      unmute: "Sesi Aç",
      read: "Okundu",
      loadingInvoice: "Fatura Detayları Yükleniyor...",
      invoiceDetails: "Fatura Detayları",
      amount: "Tutar:",
      vendor: "Satıcı / Firma:",
      category: "Kategori:",
      department: "Departman:",
      uploadedBy: "Yükleyen:",
      date: "Tarih:",
      description: "Açıklama:",
      status: "Durum:",
      pending: "Beklemede",
      paid: "Ödendi",
      payInvoice: "Faturayı Öde",
      deletedInvoiceAlert: "Bu fatura silinmiş olduğu için detayları görüntülenemiyor. Bildirim otomatik olarak temizlendi."
    },
    dashboard: {
      financialSummary: "Finansal Özet",
      income: "Ödenen Faturalar",
      expense: "Bekleyen Faturalar",
      balance: "Toplam Hacim",
      pendingCount: "Bekleyen Adet",
      dailyAnalysis: "Günlük Fatura Analizi",
      categoryDistribution: "Kategori Bazlı Fatura Dağılımı",
      recentOperations: "Son Fatura Hareketleri",
      noOperationsYet: "Henüz son fatura hareketi bulunmuyor.",
      paidStatus: "Ödendi",
      pendingStatus: "Beklemede"
    },
    invoices: {
      title: "Fatura Yönetimi",
      addInvoice: "Yeni Fatura Ekle",
      editInvoice: "Faturayı Düzenle",
      filterAll: "Tüm Faturalar",
      filterPending: "Bekleyen Faturalar",
      filterPaid: "Ödenen Faturalar",
      invoiceNumber: "Fatura No",
      invoiceNumberPlaceholder: "Boş bırakılırsa otomatik verilecek",
      amountLabel: "Tutar (₺)",
      descriptionLabel: "Açıklama",
      dateLabel: "Tarih",
      dueDateLabel: "Vade",
      daysLeft: "gün kaldı",
      daysOverdue: "gün geçti",
      dueToday: "Bugün son gün",
      vendorLabel: "Satıcı / Kurum",
      categoryLabel: "Kategori",
      departmentLabel: "Departman",
      saveInvoice: "Faturayı Kaydet",
      customCategoryPlaceholder: "Yeni kategori adı girin...",
      newCategoryAddedSuccess: "Yeni kategori başarıyla eklendi.",
      deleteConfirmTitle: "Faturayı Sil",
      deleteConfirmText: "Bu faturayı kalıcı olarak silmek istediğinize emin misiniz?",
      invoicePaidSuccess: "Fatura başarıyla ödendi.",
      invoiceDeletedSuccess: "Fatura başarıyla silindi.",
      payBtn: "Öde",
      errorLoadingInvoices: "Faturalar yüklenirken bir sorun oluştu.",
      errorLoadingCategories: "Kategoriler yüklenirken bir sorun oluştu.",
      pleaseEnterCategoryName: "Lütfen kategori adı girin.",
      updateSuccess: "Fatura başarıyla güncellendi.",
      createSuccess: "Fatura başarıyla oluşturuldu.",
      saveError: "Kaydetme işlemi sırasında bir sorun oluştu, lütfen girdiğiniz bilgileri kontrol edin.",
      pastDateError: "Geçmiş tarihli fatura girişi yapılamaz! Lütfen bugünün tarihini veya gelecekteki bir tarihi seçin.",
      deleteError: "Silme işlemi başarısız.",
      paymentError: "Ödeme işlemi başarısız oldu.",
      duplicateDetectedTitle: "Mükerrer Fatura Tespiti",
      duplicateNumberMessage: "Bu fatura numarası ({invoiceNumber}) zaten kayıtlı. Mevcut kaydı SİLİP yenisini mi eklemek istersiniz?",
      duplicateMetadataMessage: "Bu bilgilere (Tutar/Tarih/Satıcı) sahip bir fatura zaten mevcut. Mevcut kaydı DEĞİŞTİRMEK ister misiniz?",
      clearOldRecordError: "Eski kayıt sistemden temizlenemedi, lütfen tekrar deneyin.",
      saveNewRecordError: "Yeni fatura bilgileri kaydedilirken bir sorun oluştu.",
      overwriteSuccess: "Eski kayıt silindi ve yenisi başarıyla kaydedildi.",
      searchPlaceholder: "Fatura no, açıklama veya satıcı ara...",
      invoiceTypeDefault: "Fatura",
      statusPending: "Beklemede",
      statusPaid: "Ödendi",
      noInvoicesFound: "Fatura Bulunmuyor",
      noInvoicesDesc: "Henüz sisteme fatura eklenmemiş.",
      descriptionPlaceholder: "Hizmet bedeli, Amazon alımı vb.",
      vendorPlaceholder: "Örn: Trendyol, Hepsiburada",
      addCustomCategory: "+ Diğer (Yeni Kategori Ekle)",
      selectCategoryPlaceholder: "-- Kategori Seçin --",
      cannotBeUndone: "Bu işlem geri alınamaz.",
      deleteConfirmMessage: "Seçili faturayı sistemden kalıcı olarak silmek istediğinize emin misiniz?",
      payConfirmTitle: "Ödeme Onayı",
      payConfirmMessage: "Seçili fatura için ödeme işlemini onaylıyor musunuz?",
      payConfirmAmount: "Tutar:"
    },
    reports: {
      title: "Gelişmiş Raporlar",
      exportReport: "Raporu Dışa Aktar",
      pdfReport: "PDF Raporu",
      excelTable: "Excel Tablosu",
      wordDocument: "Word Belgesi",
      filename: "Dosya Adı",
      download: "İndir",
      dailyAnalysisChart: "Günlük Fatura Analizi",
      categoryDistributionChart: "Kategori Bazlı Fatura Dağılımı",
      wordProgress: "Word dökümanı hazırlanıyor...",
      excelProgress: "Excel tablosu hazırlanıyor...",
      pdfProgress: "PDF raporu hazırlanıyor..."
    },
    staff: {
      title: "Personel Yönetimi",
      addStaff: "Yeni Personel Ekle",
      fullname: "Ad Soyad",
      email: "E-posta",
      role: "Rol",
      department: "Departman",
      status: "Durum",
      active: "Aktif",
      passive: "Pasif",
      actions: "İşlemler",
      addStaffSuccess: "Personel başarıyla eklendi.",
      staffList: "Personel Listesi",
      adminRole: "Yönetici (ADMIN)",
      managerRole: "Müdür (MANAGER)",
      userRole: "Personel (USER)",
      errorLoadingStaff: "Personel listesi şu an görüntülenemiyor, lütfen sayfanızı yenileyip tekrar deneyin.",
      updateStaffSuccess: "Personel bilgileri güncellendi.",
      passwordRequired: "Yeni personel için şifre zorunludur.",
      saveError: "İşleminiz kaydedilemedi, lütfen girdiğiniz bilgilerin doğruluğunu kontrol edin.",
      deleteStaffSuccess: "Personel başarıyla silindi.",
      deleteStaffTitle: "Personeli Sil",
      deleteConfirmMessage: "isimli personeli sistemden kalıcı olarak silmek istediğinize emin misiniz?",
      updateStaffTitle: "Personel Güncelle",
      noStaffAdded: "Henüz personel eklenmemiş.",
      passwordLabelNew: "Şifre",
      passwordLabelEdit: "Şifre (Değiştirmek istemiyorsanız boş bırakın)",
      fullnamePlaceholder: "Ahmet Yılmaz",
      emailPlaceholder: "ahmet@sirket.com"
    },
    settings: {
      title: "Hesap Ayarları",
      profileSettings: "Profil Ayarları",
      securitySettings: "Güvenlik Ayarları",
      darkTheme: "Koyu Tema",
      lightTheme: "Açık Tema",
      updateProfile: "Profili Güncelle",
      updatePassword: "Şifreyi Güncelle",
      themeSettings: "Tema Seçimi",
      currentPassword: "Mevcut Şifre",
      newPassword: "Yeni Şifre",
      newPasswordConfirm: "Yeni Şifre (Tekrar)",
      saveChanges: "Değişiklikleri Kaydet",
      profileUpdatedSuccess: "Profiliniz başarıyla güncellendi.",
      passwordUpdatedSuccess: "Şifreniz başarıyla güncellendi."
    },
    departments: {
      Muhasebe: "Muhasebe",
      Finans: "Finans",
      IK: "İnsan Kaynakları (IK)",
      Satis: "Satış",
      Pazarlama: "Pazarlama",
      Yazilim: "Yazılım / IT",
      Operasyon: "Operasyon",
      Diger: "Diğer",
      unspecified: "Belirtilmemiş"
    },
    aiAssistant: {
      welcome: "Merhaba! Ben Muhasebe AI asistanıyım. Finansal verilerinizle ilgili size nasıl yardımcı olabilirim?",
      unreachable: "Şu an bağlantı kuramıyorum. Lütfen API anahtarınızı ve internetinizi kontrol edin.",
      errorOccurred: "Üzgünüm, bir hata oluştu:",
      title: "Finansal Asistan",
      online: "Çevrimiçi",
      closeBtn: "Kapat",
      placeholder: "Bir soru sorun...",
      askButtonTitle: "AI Asistana Sor"
    }
  },
  en: {
    common: {
      save: "Save",
      cancel: "Cancel",
      loading: "Loading...",
      edit: "Edit",
      delete: "Delete",
      update: "Update",
      yes: "Yes",
      no: "No",
      general: "General",
      system: "System",
      search: "Search...",
      close: "Close",
      other: "Other",
      success: "Success",
      serverUnreachable: "Server is currently unreachable, please check your internet connection.",
      deleting: "Deleting...",
      yesDelete: "Yes, Delete",
      paying: "Paying...",
      yesPay: "Yes, Pay",
      unspecified: "Not Specified"
    },
    sidebar: {
      dashboard: "Dashboard",
      invoices: "Invoices",
      staff: "Staff Management",
      reports: "Reports and Analysis",
      settings: "Settings",
      logout: "Log Out"
    },
    topbar: {
      searchPlaceholder: "Search invoices, vendors or descriptions...",
      notifications: "Notifications",
      markAllRead: "Mark All Read",
      clearAll: "Clear All",
      mute: "Mute",
      unmute: "Unmute",
      read: "Read",
      loadingInvoice: "Loading Invoice Details...",
      invoiceDetails: "Invoice Details",
      amount: "Amount:",
      vendor: "Vendor / Company:",
      category: "Category:",
      department: "Department:",
      uploadedBy: "Uploaded By:",
      date: "Date:",
      description: "Description:",
      status: "Status:",
      pending: "Pending",
      paid: "Paid",
      payInvoice: "Pay Invoice",
      deletedInvoiceAlert: "This invoice has been deleted, so details cannot be displayed. The notification was cleaned up automatically."
    },
    dashboard: {
      financialSummary: "Financial Summary",
      income: "Paid Invoices",
      expense: "Pending Invoices",
      balance: "Total Volume",
      pendingCount: "Pending Count",
      dailyAnalysis: "Daily Invoice Analysis",
      categoryDistribution: "Category-Based Distribution",
      recentOperations: "Recent Invoice Activities",
      noOperationsYet: "No recent invoice activities yet.",
      paidStatus: "Paid",
      pendingStatus: "Pending"
    },
    invoices: {
      title: "Invoice Management",
      addInvoice: "Add New Invoice",
      editInvoice: "Edit Invoice",
      filterAll: "All Invoices",
      filterPending: "Pending Invoices",
      filterPaid: "Paid Invoices",
      invoiceNumber: "Invoice No",
      invoiceNumberPlaceholder: "If left blank, it will be assigned automatically",
      amountLabel: "Amount (₺)",
      descriptionLabel: "Description",
      dateLabel: "Date",
      dueDateLabel: "Due Date",
      daysLeft: "days left",
      daysOverdue: "days overdue",
      dueToday: "Due today",
      vendorLabel: "Vendor / Company",
      categoryLabel: "Category",
      departmentLabel: "Department",
      saveInvoice: "Save Invoice",
      customCategoryPlaceholder: "Enter new category name...",
      newCategoryAddedSuccess: "New category successfully added.",
      deleteConfirmTitle: "Delete Invoice",
      deleteConfirmText: "Are you sure you want to permanently delete this invoice?",
      invoicePaidSuccess: "Invoice paid successfully.",
      invoiceDeletedSuccess: "Invoice deleted successfully.",
      payBtn: "Pay",
      errorLoadingInvoices: "A problem occurred while loading invoices.",
      errorLoadingCategories: "A problem occurred while loading categories.",
      pleaseEnterCategoryName: "Please enter a category name.",
      updateSuccess: "Invoice successfully updated.",
      createSuccess: "Invoice successfully created.",
      saveError: "An error occurred during saving, please check your inputs.",
      pastDateError: "Invoice date cannot be in the past! Please select today's date or a future date.",
      deleteError: "Deletion failed.",
      paymentError: "Payment process failed.",
      duplicateDetectedTitle: "Duplicate Invoice Detected",
      duplicateNumberMessage: "This invoice number ({invoiceNumber}) is already registered. Would you like to DELETE the existing record and add the new one?",
      duplicateMetadataMessage: "An invoice with this information (Amount/Date/Vendor) already exists. Would you like to REPLACE the existing record?",
      clearOldRecordError: "The old record could not be cleared from the system, please try again.",
      saveNewRecordError: "An error occurred while saving the new invoice details.",
      overwriteSuccess: "The old record was deleted and the new one was successfully saved.",
      searchPlaceholder: "Search invoice no, description or vendor...",
      invoiceTypeDefault: "Invoice",
      statusPending: "Pending",
      statusPaid: "Paid",
      noInvoicesFound: "No Invoices Found",
      noInvoicesDesc: "No invoices have been added to the system yet.",
      descriptionPlaceholder: "Service fee, Amazon purchase etc.",
      vendorPlaceholder: "e.g. Amazon, Apple",
      addCustomCategory: "+ Other (Add New Category)",
      selectCategoryPlaceholder: "-- Select Category --",
      cannotBeUndone: "This action cannot be undone.",
      deleteConfirmMessage: "Are you sure you want to permanently delete the selected invoice?",
      payConfirmTitle: "Payment Confirmation",
      payConfirmMessage: "Do you confirm the payment for the selected invoice?",
      payConfirmAmount: "Amount:"
    },
    reports: {
      title: "Advanced Reports",
      exportReport: "Export Report",
      pdfReport: "PDF Report",
      excelTable: "Excel Table",
      wordDocument: "Word Document",
      filename: "File Name",
      download: "Download",
      dailyAnalysisChart: "Daily Invoice Analysis",
      categoryDistributionChart: "Category-Based Distribution",
      wordProgress: "Preparing Word document...",
      excelProgress: "Preparing Excel spreadsheet...",
      pdfProgress: "Preparing PDF report..."
    },
    staff: {
      title: "Staff Management",
      addStaff: "Add New Employee",
      fullname: "Full Name",
      email: "Email Address",
      role: "Role",
      department: "Department",
      status: "Status",
      active: "Active",
      passive: "Passive",
      actions: "Actions",
      addStaffSuccess: "Employee successfully added.",
      staffList: "Employee List",
      adminRole: "Administrator (ADMIN)",
      managerRole: "Manager (MANAGER)",
      userRole: "Staff (USER)",
      errorLoadingStaff: "Staff list is currently unavailable, please refresh the page and try again.",
      updateStaffSuccess: "Staff information updated.",
      passwordRequired: "Password is required for new staff.",
      saveError: "Your transaction could not be saved, please check the correctness of your information.",
      deleteStaffSuccess: "Staff deleted successfully.",
      deleteStaffTitle: "Delete Staff",
      deleteConfirmMessage: "Are you sure you want to permanently delete the staff member named",
      updateStaffTitle: "Update Staff",
      noStaffAdded: "No staff has been added yet.",
      passwordLabelNew: "Password",
      passwordLabelEdit: "Password (Leave blank if you do not want to change)",
      fullnamePlaceholder: "John Doe",
      emailPlaceholder: "john@company.com"
    },
    settings: {
      title: "Account Settings",
      profileSettings: "Profile Settings",
      securitySettings: "Security Settings",
      darkTheme: "Dark Theme",
      lightTheme: "Light Theme",
      updateProfile: "Update Profile",
      updatePassword: "Update Password",
      themeSettings: "Theme Selection",
      currentPassword: "Current Password",
      newPassword: "New Password",
      newPasswordConfirm: "Confirm New Password",
      saveChanges: "Save Changes",
      profileUpdatedSuccess: "Your profile has been updated successfully.",
      passwordUpdatedSuccess: "Your password has been updated successfully."
    },
    departments: {
      Muhasebe: "Accounting",
      Finans: "Finance",
      IK: "Human Resources (HR)",
      Satis: "Sales",
      Pazarlama: "Marketing",
      Yazilim: "Software / IT",
      Operasyon: "Operations",
      Diger: "Other",
      unspecified: "Not Specified"
    },
    aiAssistant: {
      welcome: "Hello! I am your Accounting AI assistant. How can I help you with your financial data?",
      unreachable: "I cannot connect right now. Please check your API key and internet connection.",
      errorOccurred: "Sorry, an error occurred:",
      title: "Financial Assistant",
      online: "Online",
      closeBtn: "Close",
      placeholder: "Ask a question...",
      askButtonTitle: "Ask AI Assistant"
    }
  }
};

